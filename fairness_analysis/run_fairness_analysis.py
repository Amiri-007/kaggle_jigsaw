...
def main():
    ...
    if not args.skip_counts:
        success = run_command(
            f"python fairness_analysis/demographic_counts.py --data {args.data}",
        ...
        success = run_command(
            f"python fairness_analysis/demographic_visualization.py",
    ...
    success = run_command(
        f"python fairness_analysis/audit_fairness_v2.py --preds {preds_file} --val {args.data} --thr {args.threshold}",
    ...
    if not args.skip_intersectional:
        success = run_command(
            f"python fairness_analysis/intersectional_fairness.py --preds {preds_file} --val {args.data} --thr {args.threshold}",
    ...
    # 4. Check compliance
    success = run_command(
        "python fairness_analysis/check_compliance.py",
    ...
    if not args.skip_dashboard:
        ...
        try:
            subprocess.run(["streamlit", "run", "fairness_analysis/fairness_dashboard.py"])
        except KeyboardInterrupt:
            print("\nDashboard closed.")
        except Exception as e:
            print(f"\n⚠️ Warning: Could not launch dashboard: {e}")
            print("Try running manually with: streamlit run fairness_analysis/fairness_dashboard.py")
    else:
        print("\n✅ Analysis pipeline completed successfully!")
        print("\nTo view the interactive dashboard, run:")
        print("  streamlit run fairness_analysis/fairness_dashboard.py")
