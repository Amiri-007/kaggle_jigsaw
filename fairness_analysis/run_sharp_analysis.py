...
from src.models.bert_headtail import BertHeadTail
...
def plot_divergence_scores(divergence_scores, group_sizes, output_dir):
    ...
    df = pd.DataFrame({
        'Group': [IDENTITY_DISPLAY_NAMES.get(k, k) for k in divergence_scores.keys()],
    ...
    plt.title('SHarP Divergence: How Different is Model Reasoning for Each Group?', fontsize=14)
...
def main():
    parser = argparse.ArgumentParser(description="Run SHarP (SHAP-based Fairness) analysis")
    parser.add_argument("--model-path", default="output/checkpoints/distilbert_headtail_fold0.pth", 
                        help="Path to model checkpoint")
    parser.add_argument("--data", default="data/train.csv", help="Path to data CSV")
    parser.add_argument("--output-dir", default="output/explainability", help="Output directory")
    parser.add_argument("--sample", type=int, default=500, help="Number of examples to analyze")
    ...
    print("(via feature attributions) for each demographic group compared to the overall population.")
