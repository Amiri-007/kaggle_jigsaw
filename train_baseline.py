#!/usr/bin/env python3
"""
Jigsaw Unintended Bias Audit - Baseline Model Training Script

This script trains a TF-IDF + Logistic Regression model on the entire dataset
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Train and evaluate a bias-aware model")
    parser.add_argument("--model-name", type=str, default="tfidf_lr_full",
                        help="Name for the model and output files")
    parser.add_argument("--fast", action="store_true",
                        help="Use a small subset of data (10k rows) for quick demonstration")
...
def main():
    ...
    sample_rows = 10000 if fast_mode else None
    
    # Load train data
    train_df = pd.read_csv("data/train.csv", nrows=sample_rows)
    ...
    features = FeatureUnion([
        ('tfidf', TfidfVectorizer(
            max_features=100000 if not fast_mode else 50000,
            min_df=5,
            max_df=0.9,
    ...
    test_df = pd.read_csv("data/test_public_expanded.csv", nrows=sample_rows)
    print(f"Loaded test data with {len(test_df):,} rows")
    
    # Preprocess test data
    test_df["comment_text"] = test_df["comment_text"].fillna("").apply(preprocess_text)
    
    # Make predictions on test data
    test_preds = model.predict_proba(test_df["comment_text"])[:, 1]
    
    # Save test predictions
    test_preds_path = os.path.join("output/preds", f"{model_name}.csv")
    pd.DataFrame({"id": test_df["id"], "prediction": test_preds}).to_csv(test_preds_path, index=False)
